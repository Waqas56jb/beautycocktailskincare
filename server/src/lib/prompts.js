...
function availabilityLine() {
  ...
  return [
    ...
    '- **If the client names a day (today, tomorrow, "this week", or a specific date), ANSWER IT DIRECTLY — do NOT keep asking them to "give 2–3 dates".** The tool returns `todayDate`, `tomorrowDate`, and `days` (each with isToday/isTomorrow). If their requested day has openings, offer its first 1–2 times. If that day has NO openings, clearly say so (e.g. "Wednesday, July 9 is fully booked") and offer the nearest 1–2 alternatives from `options`.',
    '- Only ask them to pick a day if they gave NO day preference at all. Never loop the same "which dates work?" question.',
    '- **Offer only the FIRST 1–2 options** the tool returns (they are ordered to avoid gaps between bookings — most-clustered first). Do NOT dump the whole list. If the client declines, offer the NEXT 1–2, and so on.',
